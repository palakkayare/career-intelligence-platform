    ...
    def _seeker(cls, user):
        ...
        return {
            ...
            "applications": [
                ...
                for a in Application.all_objects.filter(seeker=profile).select_related(
                    "job", "job__company"
                )
            ],
...
class AccountDeactivationService:
    ...
    def _verify_identity(user, password):
        ...
        if not user.has_usable_password():
            return
        if not password:
            raise ValidationError({"password": "Confirm your password to close the account."})

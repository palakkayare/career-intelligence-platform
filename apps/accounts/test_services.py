...
from apps.accounts.models import BackupCode, OTPCode, TwoFactorAuth
from apps.accounts.services import OTPService, PendingAuthService, TwoFactorAuthService
...
def test_backup_codes_are_stored_hashed(user_with_2fa):
    ...
    stored = set(
        BackupCode.objects.filter(user=user).values_list(
            "code_hash",
            flat=True,
        )
    )

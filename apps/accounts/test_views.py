...
def test_registration_creates_an_unverified_user(api, plans):
    ...
    response = api.post('/api/v1/auth/register/', register_payload(),
                        format='json')
...
def test_nobody_can_register_themselves_as_admin(api, plans):
    ...
    """
    response = api.post(
        '/api/v1/auth/register/', register_payload(role='admin'),
        format='json',
    )
...
def test_the_right_code_verifies_the_email(api, seeker_user):
    ...
    response = api.post('/api/v1/auth/verify-email/', {
        'email': seeker_user.email, 'code': otp.code,
    }, format='json')
...
def test_a_wrong_code_leaves_the_email_unverified(api, seeker_user):
    ...
    response = api.post('/api/v1/auth/verify-email/', {
        'email': seeker_user.email, 'code': '000000',
    }, format='json')
...
def test_resend_issues_a_fresh_code(api, seeker_user):
    ...
    response = api.post('/api/v1/auth/resend-otp/', {
...
def test_a_wrong_password_is_rejected(api, seeker_user):
    response = api.post('/api/v1/auth/login/', {
        'email': seeker_user.email, 'password': 'wrong',
    }, format='json')
...
def test_an_unknown_email_gets_the_same_error_as_a_wrong_password(api, seeker_user):
    ...
    wrong_password = api.post('/api/v1/auth/login/', {
        'email': seeker_user.email, 'password': 'wrong',
    }, format='json')
    unknown_email = api.post('/api/v1/auth/login/', {
...
def test_the_email_is_matched_case_insensitively(api, seeker_user):
    response = api.post('/api/v1/auth/login/', {
        'email': seeker_user.email.upper(), 'password': PASSWORD,
    }, format='json')
...
def test_a_seeker_sees_only_their_own_history(auth, seeker_user, recruiter_user):
    LoginHistory.objects.create(
        user=recruiter_user, email_attempted=recruiter_user.email,
        status=LoginHistory.Status.SUCCESS, ip_address='10.0.0.1',
    )
    LoginHistory.objects.create(
        user=seeker_user, email_attempted=seeker_user.email,
        status=LoginHistory.Status.SUCCESS, ip_address='203.0.113.7',
    )
...
def test_the_second_step_exchanges_a_code_for_a_token(api, user_with_2fa):
    ...
    step_one = api.post('/api/v1/auth/login/', {
        'email': user.email, 'password': PASSWORD,
    }, format='json')

    response = api.post('/api/v1/auth/2fa/verify-login/', {
...
def test_step_two_cannot_be_reached_without_a_pending_token(api, user_with_2fa):
    ...
    response = api.post('/api/v1/auth/2fa/verify-login/', {
        'pending_token': 'forged', 'code': pyotp.TOTP(secret).now(),
    }, format='json')

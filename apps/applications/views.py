...
class MyApplicationsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        qs = (
            Application.all_objects.filter(seeker=self.request.user.seeker_profile)
            .select_related(
                "job",
                "job__company",
...
class ApplicationHistoryView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        is_owner = user.role == "seeker" and application.seeker.user_id == user.id
        is_recruiter = (
            user.role == "recruiter"
...
class QuotaStatusView(APIView):
    ...
    def get(self, request):
        ...
        if unlimited:
            ...
            message = (
                f"You have {usage['remaining']} applications remaining " "in the next 30 days."
            )
        else:
            message = (
...
class JobApplicationsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        qs = manager.filter(job=job).select_related("seeker", "seeker__user", "job")
        status_filter = self.request.query_params.get("status")
        if status_filter:
            qs = qs.filter(status=status_filter)

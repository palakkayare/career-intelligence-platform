...
class AuditLogAdmin(admin.ModelAdmin):
    list_display = (
        'created_at', 'user_email', 'action', 'model_name',
    ...
    readonly_fields = (
        'user', 'user_email', 'action', 'model_name', 'object_id',
        'object_repr', 'ip_address', 'user_agent', 'created_at',
        'formatted_diff',
    )
    ...
    def formatted_diff(self, obj):
        ...
        if not rows:
            return '-'
        return format_html(
            '<table><tr><th>Field</th><th>Before</th><th>After</th></tr>'
            '{}</table>',

...
from .models import SkillGapSnapshot, TargetRole, TargetRoleSkill, UserLearning
...
class SkillGapService:
    ...
    def analyze(cls, seeker: SeekerProfile, target_role: TargetRole) -> dict:
        ...
        target_skills = list(
            TargetRoleSkill.objects.filter(target_role=target_role).select_related("skill")
        )

        if not target_skills:
            logger.warning("Target role %s has no skills configured", target_role.slug)
            return {
    ...
    def get_progress(
        ...
        return list(
            SkillGapSnapshot.objects.filter(seeker=seeker, target_role=target_role).order_by(
                "-created_at"
            )[:limit]
        )
    ...
    def _collect_seeker_skill_ids(seeker: SeekerProfile) -> set:
        ...
        """
        # 1. Profile skills
        skill_ids = set(seeker.skills.values_list("id", flat=True))
        ...
        if primary_resume:
            resume_skill_ids = set(
                primary_resume.resume_skills.filter(is_confirmed=True).values_list(
                    "skill_id", flat=True
                )
            )
    ...
    def _serialize_role(target_role: TargetRole) -> dict:
        ...
        return {
            ...
            "avg_salary_inr": (
                str(target_role.avg_salary_inr) if target_role.avg_salary_inr is not None else None
            ),
        }

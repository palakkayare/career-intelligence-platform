...
class FakeResource:
    ...
    def __init__(self, id, difficulty="beginner", quality_score=50, is_endorsed=False):
        self.id = id
...
def test_finished_courses_are_not_recommended_again():
    ranked = alg.rank_resources(
        [FakeResource(1), FakeResource(2)],
        user_completed_ids={1},
    )
...
def test_the_total_is_capped_too():
    ...
    items = [{"skill": {"skill_id": i}, "resources": list(range(3))} for i in range(10)]
...
@pytest.mark.django_db
def test_nothing_is_recommended_when_the_gap_is_closed(seeker_user, learning_setup):
...
@pytest.mark.django_db
@pytest.mark.regression
def test_restarting_a_course_does_not_inflate_enrollments(seeker_user, learning_setup):
...
@pytest.mark.django_db
@pytest.mark.regression
def test_reaching_100_percent_completes_the_course(seeker_user, learning_setup):
...
@pytest.mark.django_db
def test_any_progress_moves_a_wishlist_item_into_progress(seeker_user, learning_setup):
    _, _, resources = learning_setup

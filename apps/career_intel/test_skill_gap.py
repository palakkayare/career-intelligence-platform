...
def test_the_suggestion_list_is_capped():
    ...
    split = alg.split_skills(set(), [
        fake(i, f'Skill {i}', 'critical', 'easy') for i in range(20)
    ])
...
@pytest.mark.django_db
def test_the_recommendations_start_with_the_easy_critical_skill(seeker,
                                                                target_role):
    role, _ = target_role
...
def test_a_role_with_no_skills_yet_returns_a_full_shape(seeker, db):
    ...
    assert result['missing_skills'] == {
        'critical': [], 'important': [], 'preferred': [], 'optional': [],
    }

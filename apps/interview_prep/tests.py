...
def test_company_tips_are_scoped_to_that_company(company, plans, industry, recruiter_user):
    ...
    CompanyInterviewTip.objects.create(
        company=company,
        category=CompanyInterviewTip.Category.PROCESS,
        tip="Three rounds.",
    )
    CompanyInterviewTip.objects.create(
        company=other,
...
def test_an_item_can_be_unticked(seeker_user):
    ...
    assert (
        InterviewPrepService.checklist(
            seeker_user,
            "Test Corp",
        )["completed_items"]
        == 0

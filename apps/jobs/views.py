...
class JobDetailView(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        ...
        base = Job.objects.select_related('company', 'category', 'posted_by').filter(is_deleted=False)
        if hasattr(user, 'recruiter_profile'):
            return base.filter(
                Q(status=Job.Status.ACTIVE) |
                Q(posted_by=user.recruiter_profile)
            )
    ...
    def perform_update(self, serializer):
        ...
        if not instance.can_be_edited_by(self.request.user):
            raise PermissionDenied(
                "Only DRAFT or REJECTED jobs can be edited."
            )
        serializer.save()

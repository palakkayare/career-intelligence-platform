...
class CandidateSearchService:
    ...
    @classmethod
    def search(cls, recruiter, filters: dict, target_job_id=None, page=1, page_size=20):
        """
        ...
        seekers = list(
            qs[offset : offset + page_size].select_related("user").prefetch_related("skills")
        )
    ...
    def _apply_filters(cls, qs, filters):
        ...
        if filters.get("experience_years_min") is not None:
            qs = qs.filter(years_of_experience__gte=filters["experience_years_min"])

        if filters.get("experience_years_max") is not None:
            qs = qs.filter(years_of_experience__lte=filters["experience_years_max"])
    ...
    def _order_by_match_score(cls, qs, target_job_id):
        ...
        candidate_ids = list(qs.values_list("id", flat=True)[:MATCH_SCORE_RANKING_LIMIT])
        if not candidate_ids:
            return qs.none()

        scored_ids = list(
            MatchScore.objects.filter(seeker_id__in=candidate_ids, job=job)
        ...
        preserved_order = Case(
            *[When(pk=sid, then=Value(index)) for index, sid in enumerate(final_order)],
...
class CandidateProfileService:
    ...
    def reveal_contact(cls, recruiter, seeker):
        ...
        credits = RecruiterCredits.objects.select_for_update().filter(recruiter=recruiter).first()
        if credits is None:
    ...
    def _notify_seeker(recruiter, seeker):
        ...
        company_name = recruiter.company.name if recruiter.company_id else "a company"
        try:

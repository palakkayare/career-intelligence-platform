...
def test_a_pool_with_no_filters_holds_everyone_discoverable(recruiter, plans):
    catch_all = TalentPool.objects.create(
        recruiter=recruiter, name='Everyone', filters={},
    )
...
def test_new_arrivals_notify_the_recruiter(pool, recruiter):
    ...
    assert Notification.objects.filter(
        user=recruiter.user, kind=NotificationKind.NEW_MATCHING_CANDIDATE,
    ).exists()
...
def test_the_sweep_covers_every_pool(recruiter, plans):
    ...
    TalentPool.objects.create(
        recruiter=recruiter, name='Bangalore',
        filters={'location_city': 'Bangalore'},
    )
    TalentPool.objects.create(
        recruiter=recruiter, name='Pune',
...
def test_the_sweep_skips_muted_pools(recruiter, plans):
    ...
    TalentPool.objects.create(
        recruiter=recruiter, name='Muted', filters={}, notify_on_new=False,
    )
    make_seeker('anyone@test.com')
...
def business_recruiter(recruiter, plans):
    ...
    business = Plan.objects.create(
        name='Business Monthly', slug='business_monthly',
    ...
    if sub is None:
        Subscription.objects.create(
            user=recruiter.user, plan=business,
...
def test_a_recruiter_can_create_a_pool(recruiter_client):
    response = recruiter_client.post('/api/v1/candidates/pools/', {
        'name': 'Senior Python', 'filters': {'location_city': 'Bangalore'},
    }, format='json')
...
def test_a_recruiter_only_sees_their_own_pools(recruiter_client, pool, plans):
    other_user = User.objects.create_user(
        email='other-recruiter@test.com', password='TestPass123!',
        role=User.Role.RECRUITER, is_email_verified=True,
    )
    TalentPool.objects.create(
        recruiter=other_user.recruiter_profile, name='Theirs', filters={},
    )

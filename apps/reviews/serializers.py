...
class CompanyReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CompanyReview
        fields = (
            'id', 'job_title', 'employment_status', 'employment_years',
            'rating_culture', 'rating_management', 'rating_growth',
            'rating_salary', 'overall_rating',
            'headline', 'pros', 'cons', 'advice_to_management',
            'would_recommend', 'is_verified_employee',
            'helpful_count', 'company_response', 'is_mine', 'created_at',
        )
        read_only_fields = (
            'id', 'is_verified_employee', 'helpful_count',
            'company_response', 'created_at',
        )
...
class CompanyReviewWriteSerializer(serializers.ModelSerializer):
    class Meta:
        model = CompanyReview
        fields = (
            'job_title', 'employment_status', 'employment_years',
            'rating_culture', 'rating_management', 'rating_growth',
            'rating_salary', 'headline', 'pros', 'cons',
            'advice_to_management', 'would_recommend',
        )
...
class InterviewExperienceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            'id', 'role_applied', 'outcome', 'difficulty', 'rounds',
            'weeks_to_decision', 'process', 'questions_asked',
            'was_experience_positive', 'is_mine', 'created_at',
        )
...
class InterviewExperienceWriteSerializer(serializers.ModelSerializer):
    class Meta:
        model = InterviewExperience
        fields = (
            'role_applied', 'outcome', 'difficulty', 'rounds',
            'weeks_to_decision', 'process', 'questions_asked',

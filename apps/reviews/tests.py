...
def test_the_same_person_can_review_two_companies(author, company, industry, recruiter_user):
...
def test_verification_cannot_be_set_by_the_submitter(auth, company):
    ...
    response = auth.post(
        ...
        {**REVIEW_DATA, "is_verified_employee": True},
        format="json",
    )
...
def test_the_queue_leads_with_the_most_reported(company, plans):
    ...
    loud = post_review(make_seeker("loud-author@test.com"), company, headline="Second review")
...
def test_a_recruiter_can_respond_to_a_review_of_their_company(review, recruiter, company):
...
@pytest.mark.regression
def test_a_recruiter_cannot_respond_to_another_companys_review(review, recruiter):
...
def test_the_response_survives_the_responder_leaving(review, recruiter, company):
    recruiter.company = company
    recruiter.save()
    response = ResponseService.respond(review, recruiter, "Thanks for this.")
...
@pytest.mark.regression
def test_the_same_person_can_share_two_interviews_at_one_company(author, company):
    """
    ...
    second = InterviewExperienceService.create(
        author,
        company,
        {**EXPERIENCE_DATA, "role_applied": "Senior Backend"},
    )
...
def test_the_interview_summary_reports_the_shape_of_the_process(company, plans):
    InterviewExperienceService.create(
        make_seeker("i1@test.com"),
        company,
        {**EXPERIENCE_DATA, "rounds": 3},
    )
    InterviewExperienceService.create(
        make_seeker("i2@test.com"),
        company,
        {
            **EXPERIENCE_DATA,
            "rounds": 5,

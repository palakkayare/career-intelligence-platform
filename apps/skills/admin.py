...
class SkillAdmin(admin.ModelAdmin):
    list_display = ('name', 'category', 'alias_list', 'usage_count',
                    'is_approved', 'is_deprecated')
    ...
    def merge_into_first(self, request, queryset):
        ...
        if len(skills) < 2:
            self.message_user(
                request, 'Select at least two skills to merge.',
        ...
        ranked = sorted(skills, key=lambda s: s.seeker_skills.count(),
                        reverse=True)
        ...
        for source in sources:
            try:
                ...
                self.message_user(
                    request, f'Could not merge {source.name}: {exc}',

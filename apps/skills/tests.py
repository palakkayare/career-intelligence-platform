...
def make_skill(name, aliases=None, deprecated=False):
    return Skill.objects.create(
        name=name, aliases=aliases or [], is_deprecated=deprecated,
    )
...
def test_the_command_can_merge_one_named_pair():
    ...
    call_command(
        'merge_duplicate_skills', source='ReactJS', target='React',
        verbosity=0,
    )
...
def test_the_command_refuses_an_unknown_skill():
    ...
    with pytest.raises(CommandError):
        call_command(
            'merge_duplicate_skills', source='Nonsense', target='React',
            verbosity=0,
        )
